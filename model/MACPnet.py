...
"""
The following is the network structure of MACPnet, which enjoys the original rights to the following code
@author:Dengxun
@Email:dengxun22@mails.ucas.ac.cn
"""
...
class maxvit(nn.Module):
    def __init__(self,imgsize):
        super().__init__()
        model = maxvit_large_tf_512(pretrained=True)
        #print(model)
        stem = model.stem
        stages1 = model.stages[0].blocks
...
class MAC_block(nn.Module):
    ...
    def forward(self,x):
        ...
        xconv_3_7=self.maxpool(xconv3)
        xcombine=self.conv(torch.cat([xconv_3_7,x_max],dim=1))
...
class MACPnet(nn.Module):
    """
    ================================================================
    Total params: 59,780,837
    Trainable params: 59,780,837
    Non-trainable params: 0
    ----------------------------------------------------------------
    Input size (MB): 1.00
    Forward/backward pass size (MB): 4641.00
    Params size (MB): 228.05
    Estimated Total Size (MB): 4870.05
    ----------------------------------------------------------------
    """
    ...
    def __init__(self):
        ...
        self.MAC256=MAC_block(inc=128, outc=128, imgsize=256)
        self.MAC128 = MAC_block(inc=128, outc=256, imgsize=128)
        self.MAC64 = MAC_block(inc=256, outc=512, imgsize=64)
        self.MAC32 = MAC_block(inc=512, outc=1024, imgsize=32)
    ...
    def forward(self,x):
        ...
        xdown128=self.MAC256(xdown256)
        xdown64 = self.MAC128(xdown128)
        xdown32 = self.MAC64(xdown64)
        xdown16=self.MAC32(xdown32)
...
#     model=MACPnet()
#     torchsummary.summary(model,(1,512,512),batch_size=1)
#     print(model)
